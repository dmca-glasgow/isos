...
export function CalloutIcon(props: HTMLAttributes<HTMLSpanElement>) {
  const className = String(props.class || '');
  const match = className.match(/callout-icon-(.+)/);
  if (match) {
    return <IconSwitch type={match[1]} />;
  }
  return null;
}

function IconSwitch({ type }: { type: string }) {
  switch (type) {
    case 'note':
      return <NoteIcon />;
    case 'tip':
