...
export const unifiedLatexProcessMacrosAndEnvironmentsWithMathReparse: Plugin<
  ...
  Ast.Root,
  Ast.Root
> = function unifiedLatexProcessMacrosAndEnvironmentsWithMathReparse(
  options,
) {
  ...
  const isRelevantEnvironment =
    match.createEnvironmentMatcher(environments);
  const isRelevantMathEnvironment =
    match.createEnvironmentMatcher(mathEnvs);
